...
class Transformer(nn.Module):
    def __init__(self, min_size, max_size, device, stride=32, ):
        super().__init__()
    ...
    def horizontal_flip(self, image, boxes):
        img_center = torch.Tensor(list(image.shape[1:])[::-1]) / 2
        img_center = torch.hstack((img_center, img_center)).to(self.device)
        image = image.flip(2)
